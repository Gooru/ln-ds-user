...
import org.gooru.ds.user.processor.atc.pvc.competency.CompetencyPath;
...
import org.gooru.ds.user.processor.atc.pvc.competency.ProgressionLevel;
...
class CompetencyRouteModelBuilder {
  ...
  CompetencyRouteModel build(SubjectCode subjectCode, CompetencyRoute competencyRoute) {
    ...
    domainsAsString =
        competencyRoute.getDomains().stream().map(DomainCode::getCode).collect(Collectors.toList());
