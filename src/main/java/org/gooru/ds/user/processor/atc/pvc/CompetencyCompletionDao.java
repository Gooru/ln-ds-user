...
interface CompetencyCompletionDao {
	   ...
	   List<CompetencyCompletionModel> fetchCompetencyCompletion (@Bind("user") String user, @Bind("subjectCode") String subjectCode, 
	   ...
	   @Mapper(CompetencyCompletionModelMapper.class)
		@SqlQuery("select distinct(cm.tx_comp_code), cm.tx_domain_code, cm.tx_comp_seq, "
				...
				+ " order by cm.tx_domain_code,"
				+ " cm.tx_comp_seq asc")
		List<CompetencyCompletionModel> fetchCompetencyCompletionMonthBased(@Bind("user") String user, @Bind("subjectCode") String subjectCode, 
				   @Bind("competencyCodes") PGArray<String> competencyCodes, @Bind("month") Integer month, @Bind("year") Integer year);
