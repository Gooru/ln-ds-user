...
public class UserPerfAsmtSummaryCommand {
  ...
  private void validate() {
    ...
    if (courseId == null) {
      ...
      // throw new HttpResponseWrapperException(HttpConstants.HttpStatus.BAD_REQUEST, "Course not
      // provided");
    }

    if (unitId == null) {
      LOGGER.info("Unit not provided");
      // throw new HttpResponseWrapperException(HttpConstants.HttpStatus.BAD_REQUEST, "Unit not
      // provided");
    }

    if (lessonId == null) {
      LOGGER.info("Lesson not provided");
      // throw new HttpResponseWrapperException(HttpConstants.HttpStatus.BAD_REQUEST, "Lesson not
      // provided");
    }
