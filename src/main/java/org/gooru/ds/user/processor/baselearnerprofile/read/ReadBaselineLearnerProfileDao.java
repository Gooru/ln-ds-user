...
public interface ReadBaselineLearnerProfileDao {
  ...
  @SqlQuery(
      "select lp_data from learner_profile_baselined WHERE class_id = :classId AND course_id = :courseId "
  ...
  @SqlQuery(
      "select lp_data from learner_profile_baselined WHERE class_id is null AND course_id = :courseId "
          + " AND user_id = :user AND tx_subject_code = :subject")
