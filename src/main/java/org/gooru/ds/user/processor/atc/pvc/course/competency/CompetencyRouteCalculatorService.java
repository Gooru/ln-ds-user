...
class CompetencyRouteCalculatorService implements CompetencyRouteCalculator {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(CompetencyRouteCalculatorService.class);
  ...
  public CompetencyRouteModel calculateCompetencyRoute(RouteCalculatorModel model) {
    ...
    try {
      ...
      LOGGER.warn("Not able to compute route for class: '{}', course: '{}' and user: '{}'",
          model.getClassId(), model.getCourseId(), model.getUserId());
      throw re;
    }
  }
  ...
  private CompetencyRouteModel doProcess() {
    try {
      ...
      return new CompetencyRouteModelBuilder(getTaxonomyDao()).build(new SubjectCode(subjectCode),
  ...
  private void calculateCompetencyRoute() {
    ...
    competencyRouteToDestination = proficiencyCompetencyMap.getCeilingLine()
        .getRouteToCompetencyLine(destinationCompetencyMap.getCeilingLine());
  }
  ...
  private void fetchUserProficiencyInSpecifiedSubjectAndDomains() {
    ...
    sourceCompetencies = getTaxonomyDao().fetchProficiencyForUserInSpecifiedSubjectAndDomains(
        model.getUserId().toString(), subjectCode,
  ...
  private void filterGutCodesForCompetencyForSpecifiedSubject() {
    ...
    destinationCompetencies = getTaxonomyDao().transformGutCodesToCompetency(subjectCode,
  ...
  private void validateClassCourse() {
    ClassCourseValidatorDao classCourseValidatorDao =
        coreDSDbi.onDemand(ClassCourseValidatorDao.class);
    if (model.getClassId() != null) {
      if (!classCourseValidatorDao.validateClassCourse(model.getClassId(), model.getCourseId())) {
        ...
        throw new IllegalStateException("Invalid class/course for request, class:"
            + model.getClassId() + ", course: " + model.getCourseId());
      }
    } else {
