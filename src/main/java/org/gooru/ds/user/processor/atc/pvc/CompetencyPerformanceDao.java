...
interface CompetencyPerformanceDao {
	...
	@SqlQuery("select AVG(compPerf.scoreInPercentage) AS scoreInPercentage FROM (SELECT DISTINCT ON (gut_code) FIRST_VALUE(collection_score) "
			...
			+ "AND user_id = :user AND collection_type = 'assessment' AND extract(month from updated_at) <= :month AND "
			+ "extract(year from updated_at) <= :year ORDER BY gut_code, updated_at DESC) AS compPerf")
	Double fetchGradeCompetencyPerformanceTimeBased (@Bind("user") String user, 
