...
class UserSkylineCompetencyNextService {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(UserSkylineCompetencyNextService.class);
  ...
  Map<String, String> fetchUserSkyline(UserSkylineCompetencyNextCommand command) {
    ...
    final List<CompetencyModel> userCompetencyStatusModels =
        userCompetencySkylineDao.fetchUserCompetencyStatus(command.asBean());
    ...
    if (userCompetencyStatusModels.isEmpty()) {
      ...
      for (Map.Entry<String, CompetencyModel> entry : competencySkylineMap.entrySet()) {
        ...
        LOGGER
            .info("Get Competency Code from CompetencySkylineMap" + compModel.getCompetencyCode());
      ...
      allDomains = Collections
          .unmodifiableList(new ArrayList<>(orderedDomainCompetencyMatrixMap.getDomains()));
