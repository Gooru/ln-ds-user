...
public class CourseBasedPerfVsCompletionCalculator {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(CourseBasedPerfVsCompletionCalculator.class);
  ...
  public JsonArray fetchCourseBasedUserPerformanceCompletion(
    ...
    CompetencyCompletionService competencyCompletionService =
        new CompetencyCompletionService(DBICreator.getDbiForDefaultDS());
    ...
    if (classMembers != null && !classMembers.isEmpty()) {
      ...
      if (courseCompList != null && !courseCompList.isEmpty()) {
        ...
        for (String cm : classMembers) {
          ...
          counts = competencyCompletionService.fetchUserCompetencyStatus(cm,
              command.getSubjectCode(), courseCompList, command.getMonth(), command.getYear());
          if (!counts.isEmpty() && counts != null) {
            ...
            userPvC.put(PERCENT_COMPLETION,
                totalCompetencies != 0 ? (Double.valueOf(compCount / totalCompetencies) * 100) : 0);

            if (courseCompList != null && !courseCompList.isEmpty()
                && (command.getMonth() == null || command.getYear() == null)) {
            ...
            } else if (courseCompList != null && !courseCompList.isEmpty()
                && (command.getMonth() != null && command.getYear() != null)) {
