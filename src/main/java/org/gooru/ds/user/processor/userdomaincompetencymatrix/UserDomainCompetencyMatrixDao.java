...
interface UserDomainCompetencyMatrixDao {
  ...
  @SqlQuery("select cm.tx_domain_code, cm.tx_comp_code, cm.tx_comp_name, cm.tx_comp_desc, cm.tx_comp_student_desc, cm.tx_comp_seq, ucm.status, ucm.profile_source from"
  ...
  @SqlQuery("select distinct(cm.tx_comp_code), cm.tx_domain_code, cm.tx_comp_name, cm.tx_comp_desc, cm.tx_comp_student_desc, cm.tx_comp_seq,"
      ...
      + " as status, (SELECT DISTINCT ON (lpcs.gut_code) FIRST_VALUE(lpcs.profile_source) OVER (PARTITION BY lpcs.gut_code ORDER BY lpcs.updated_at desc) "
      + " FROM learner_profile_competency_status_ts lpcs where lpcs.user_id = :user and lpcs.gut_code = ucm.gut_code and lpcs.updated_at < :toDate) "
      + " as source from domain_competency_matrix cm left join learner_profile_competency_status_ts"
