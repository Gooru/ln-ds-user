...
//			+ " FROM learner_profile_competency_status_ts lpcs where lpcs.gut_code = ucm.gut_code and extract(month from lpcs.updated_at) <= :month"
...
//			+ " ucm.updated_at) <= :month and extract(year from ucm.updated_at) <= :year where cm.tx_subject_code = :subject order by cm.tx_domain_code,"
//			+ " cm.tx_comp_seq asc")
