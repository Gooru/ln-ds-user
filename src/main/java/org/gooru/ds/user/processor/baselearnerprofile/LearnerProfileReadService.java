...
public class LearnerProfileReadService {
  ...
  public ExecutionStatus fetchCurrentLearnerProfileAndSetBaseline(
    ...
    List<LearnerProfileReadModel> models =
        learnerProfileReadDao.fetchCurrentLearnerProfile(command.asBean());
    ...
    if (command.getClassId() != null) {
      baseLearnerProfileDao.UpdateLearnerBaselineProfile(models, command.getClassId(),
          command.getCourseId(), createdAt);
    } else if (command.getClassId() == null) {
      baseLearnerProfileDao.UpdateLearnerBaselineProfileForIL(models, command.getCourseId(),
          createdAt);
    }
