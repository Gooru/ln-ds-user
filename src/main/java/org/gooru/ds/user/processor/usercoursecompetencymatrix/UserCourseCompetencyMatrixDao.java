...
//    		+ " status FROM learner_profile_competency_status_ts lpcs where lpcs.gut_code = ucm.gut_code and extract(month from lpcs.updated_at)"
//    		+ " <= :month and extract(year from lpcs.updated_at) <= :year) as status from course_competency_matrix cm left"
...
//    		+ " ucm.user_id = :user and extract(month from ucm.updated_at) <= :month AND extract(year from updated_at) <= :year where"
