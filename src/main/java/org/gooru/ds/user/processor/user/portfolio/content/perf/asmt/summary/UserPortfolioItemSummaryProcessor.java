 ...
 * @author renuka
 */
public class UserPortfolioItemSummaryProcessor implements MessageProcessor {

  private final Vertx vertx;
  private final Message<JsonObject> message;
  private final Future<MessageResponse> result;
  private static final Logger LOGGER = LoggerFactory.getLogger(UserPortfolioItemSummaryProcessor.class);
  private EventBusMessage eventBusMessage;
  private final UserPortfolioItemSummaryService userPortfolioItemSummaryService =
      new UserPortfolioItemSummaryService(DBICreator.getDbiForAnalyticsDS(),
          DBICreator.getDbiForCoreDS());

  public UserPortfolioItemSummaryProcessor(Vertx vertx, Message<JsonObject> message) {
    this.vertx = vertx;
  ...
  private void fetchUserPortfolioUniqueItemPerf(UserPortfolioItemSummaryCommand command) {
    try {
      UserPortfolioItemSummaryModelResponse outcome =
          userPortfolioItemSummaryService.fetchUserPortfolioUniqueItemPerf(command);
