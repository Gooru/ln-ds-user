...
interface UserPerfCompetencyAssessmentsDao {
   ...
   * + "collection_average_reaction" + " from competency_collection_performance where " +
   ...
   * 
   * @BindBean UserPerfCompetencyAssessmentsCommand.UserPerfCompetencyAssessmentsCommandBean
   * userPerfCompetencyAssessmentsCommandBean);
   */

  @Mapper(UserPerfCompetencyAssessmentsModelMapper.class)
  @SqlQuery("select collection_id, latest_session_id, collection_score from learner_profile_competency_evidence where "
      + " competency_id = :competencyId and collection_type = 'assessment' and user_id = :user")
  List<UserPerfCompetencyAssessmentsModel> fetchUserPerfCompetencyAssessments(
