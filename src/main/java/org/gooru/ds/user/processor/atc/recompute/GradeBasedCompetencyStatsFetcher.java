...
public class GradeBasedCompetencyStatsFetcher {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(GradeBasedCompetencyStatsFetcher.class);
	...
	public List<CompetencyStatsModel> fetchGradeBasedUserPerformanceCompletion(GradeBasedCompetencyStatsCommand command) {
		...
		if (classMembers != null && !classMembers.isEmpty()) {
					if ((command.getMonth() == null || command.getYear() == null)) {
						userCompetencyStatsList = competencyStatsDao.fetchGradeCompetencyStats(PGArrayUtils.convertFromListStringToSqlArrayOfString(classMembers), 
								command.getClassId(), command.getSubjectCode());
					} else if ((command.getMonth() != null && command.getYear() != null)) {
						userCompetencyStatsList = competencyStatsDao.fetchGradeCompetencyStatsTimeBound(PGArrayUtils.convertFromListStringToSqlArrayOfString(classMembers),
