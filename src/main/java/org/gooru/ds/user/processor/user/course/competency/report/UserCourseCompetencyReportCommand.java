...
public class UserCourseCompetencyReportCommand {
	...
	static UserCourseCompetencyReportCommand build(JsonObject requestBody) {
		UserCourseCompetencyReportCommand command = UserCourseCompetencyReportCommand.buildFromJsonObject(requestBody);
		command.validate();
	...
	private void validateFromParams() {
		if (fromMonth == null || fromYear == null) {
			...
			throw new HttpResponseWrapperException(HttpConstants.HttpStatus.BAD_REQUEST, "Invalid fromMonth/fromYear");
		}
