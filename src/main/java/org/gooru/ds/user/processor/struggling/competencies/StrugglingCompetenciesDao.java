...
public interface StrugglingCompetenciesDao {
  ...
  @SqlQuery("SELECT id, grade, grade_seq, description, fw_code FROM grade_master WHERE id = ANY(:gradeIds::bigint[])")
  public List<GradeModel> fetchGrades(@Bind("gradeIds") PGArray<Long> gradeIds);

  @Mapper(GradeCompetencyMapModelMapper.class)
  @SqlQuery("SELECT tx_domain_id, tx_domain_code, tx_domain_seq, tx_comp_code, tx_comp_name, tx_comp_student_desc, tx_comp_seq,"
