...
public class RouteUserDetailsConfigurator implements RouteConfigurator {
  ...
  private void userGrades(RoutingContext routingContext) {
    RouteHandlerUtils.baseHandler(eb, routingContext, Constants.Message.MSG_OP_USER_GRADES,
        Constants.EventBus.MBEP_DISPATCHER, mbusTimeout, LOGGER);
  }

  private void userProfile(RoutingContext routingContext) {
    RouteHandlerUtils.baseHandler(eb, routingContext, Constants.Message.MSG_OP_USER_PROFILE,
        Constants.EventBus.MBEP_DISPATCHER, mbusTimeout, LOGGER);
  }
