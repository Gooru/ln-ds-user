...
import org.gooru.ds.user.responses.auth.AuthSessionResponseHolder;
import org.gooru.ds.user.responses.auth.AuthSessionResponseHolderBuilder;
  ...
  public void configureRoutes(Vertx vertx, Router router, JsonObject config) {
    ...
    router.route(Constants.Route.API_AUTH_ROUTE).handler(routingContext -> {
      ...
      if (sessionToken == null || sessionToken.isEmpty()) {
        ...
        // If the session token is present, we send it to Message Bus for validation. We stash it on
        // to
        ...
        eBus.<JsonObject>send(Constants.EventBus.MBEP_AUTH, null, options, reply -> {
          if (reply.succeeded()) {
            ...
            // successful auth may have been for anonymous user. We allow only GET request for
            // anonymous
            ...
            if (responseHolder.isAuthorized() && !responseHolder.isAnonymous()) {
              ...
              routingContext.put(Constants.Message.PROCESSING_HANDLER_START_TIME,
