...
public class RouteUserJourneyConfigurator implements RouteConfigurator {
  ...
  public void configureRoutes(Vertx vertx, Router router, JsonObject config) {
    ...
    router.get(Constants.Route.API_USER_PERF_COMPETENCY_COLLECTIONS)
        .handler(this::userPerfCompetencyCollections);
  }

  private void userJourney(RoutingContext routingContext) {
    RouteHandlerUtils.baseHandler(eb, routingContext, Constants.Message.MSG_OP_USER_JOURNEY,
  ...
  private void userPerfCollections(RoutingContext routingContext) {
    RouteHandlerUtils.baseHandler(eb, routingContext,
        Constants.Message.MSG_OP_USER_PERF_COLLECTIONS, Constants.EventBus.MBEP_DISPATCHER,
        mbusTimeout, LOGGER);
  }

  private void userSummaryAssessment(RoutingContext routingContext) {
    RouteHandlerUtils.baseHandler(eb, routingContext,
        Constants.Message.MSG_OP_USER_SUMMARY_ASSESSMENT, Constants.EventBus.MBEP_DISPATCHER,
        mbusTimeout, LOGGER);
  }

  private void userSummaryCollection(RoutingContext routingContext) {
    RouteHandlerUtils.baseHandler(eb, routingContext,
        Constants.Message.MSG_OP_USER_SUMMARY_COLLECTION, Constants.EventBus.MBEP_DISPATCHER,
        mbusTimeout, LOGGER);
  }
  ...
  private void userPerfCompetencyCollections(RoutingContext routingContext) {
    RouteHandlerUtils.baseHandler(eb, routingContext,
        Constants.Message.MSG_OP_USER_PERF_COMPETENCY_COLLECTIONS,
        Constants.EventBus.MBEP_DISPATCHER, mbusTimeout, LOGGER);
  }
